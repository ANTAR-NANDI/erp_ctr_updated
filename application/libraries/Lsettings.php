...
class Lsettings
{
  ...
  public function pos_setting_list()
  {
    $CI = &get_instance();
    ...
    $pos_setting_list = $CI->Settings->read_all_pos_setting();

    $length_of_pos_setting_list = count($pos_setting_list);

    $i = 0;
    foreach ($pos_setting_list as $k => $v) {
      $i++;
      $pos_setting_list[$k]['sl'] = $i;
    }

    $data = [
      'title' => display('pos_setting'),
      'pos_setting_list' => $pos_setting_list,
      'length_of_pos_setting_list' => $length_of_pos_setting_list,
      'language'          => $language,
    ];

    return $CI->parser->parse('settings/pos_setting', $data, true);
  }
