...
class Lrqsn
{
    ...
    public function stock_report_outlet_item_expiry()
    {
        $CI = &get_instance();
        $CI->load->model('Rqsn');
        $CI->load->model('Web_settings');
        ...
        $data = array(
            ...
            'discount_type' => $currency_details[0]['discount_type'],
            'currency' => $currency_details[0]['currency'],
            'totalnumber' => $CI->Reports->totalnumberof_purchase(),
            'taxes'         => $taxfield,
            'purchase_id'         => $CI->Reports->all_purchase_id()
        );


        //                echo print_r($sku_list);
        //                die();
        $invoiceForm = $CI->parser->parse('rqsn/expiry_outlet_stock', $data, true);
        return $invoiceForm;
    }
