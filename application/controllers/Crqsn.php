...
class Crqsn extends CI_Controller
{
    ...
    public function isactive($id = null, $action = null, $product_id = null, $value = null)
    {
        $CI = &get_instance();
        $CI->load->model('Rqsn');
        $CI->load->model('Reports');
    ...
    public function rqsn_delete_center($rqsn_id, $product_id)
    {
        if ($this->Rqsn->delete_rqsn_center($rqsn_id, $product_id)) {
            $this->session->set_flashdata('message', display('successfully_delete'));
        } else {
            $this->session->set_flashdata('error_message', display('please_try_again'));
        }
    ...
    public function isreceive($rqsn_id = null, $product_id = null, $action = null)
    {
        ...
        $all_items = $this->db->select('rqsn_detail_id')
            ->from('rqsn_details')
        ...
        if (!empty($all_items)) {
            foreach ($all_items as $all_item) {
                $id = $all_item->rqsn_detail_id;
                $item_total = $this->db->select('item_total,is_return')->from('rqsn_details')->where('rqsn_detail_id', $id)->get()->row();
                ...
                $postData = array(
                    ...
                    'status' => $action,
                    'isrcv' => 1,
                    'is_return' => ($item_total->is_return == 1 ? 2 : 0),
                );
                ...
                if ($item_total->item_total > 0) {
                    ...
                    $incCr = array(
                        ...
                        'Credit'          => (!empty($item_total->item_total) ? $item_total->item_total : 0),
                        'Debit'         =>  0,
                        'IsPosted'       =>  1,
                        'CreateBy'       => $user_id,
                        'CreateDate'     => $Vdate,
                    ...
                    $curDr = array(
                        ...
                        'Credit'          =>  0,
                        'Debit'         => (!empty($item_total->item_total) ? $item_total->item_total : 0),
                        'IsPosted'       =>  1,
                        'CreateBy'       => $user_id,
                        'CreateDate'     => $Vdate,
                        'IsAppove'       => 1
        ...
        if ($total_success == count($all_items)) {
            $this->session->set_flashdata('message', display('successfully_approved'));
        } else {
            $this->session->set_flashdata('error_message', display('please_try_again'));
        }
    ...
    public function outlet_rqsn_delete($rqsn_id, $product_id)
    {
        if ($this->Rqsn->delete_rqsn($rqsn_id, $product_id)) {
            $this->session->set_flashdata('message', display('successfully_delete'));
        } else {
            $this->session->set_flashdata('error_message', display('please_try_again'));
        }
    ...
    public function expiry_stock()
    {
        ...
        $content = $this->lrqsn->stock_report_outlet_item_expiry();
        // echo 'Okay';die();
        $this->template->full_admin_html_view($content);
    }
    ...
    public function outlet_stock()
    {
        ...
        $data = $this->Rqsn->outlet_stock2($postData, null, $from_date, $to_date);
        echo json_encode($data);
    }

    public function expiry_outlet_stock()
    {
        $this->load->model('Rqsn');
        $from_date = $this->input->post('from_date');
        $to_date = $this->input->post('to_date');
        $postData = $this->input->post();
        $data = $this->Rqsn->expiry_outlet_stock_report_new($postData, null, $from_date, $to_date);
        echo json_encode($data);
    }
