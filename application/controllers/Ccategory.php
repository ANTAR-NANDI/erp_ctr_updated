...
class Ccategory extends CI_Controller
{
    ...
    public function membersTree($parent_key)
    {
        ...
        //     $row1[$key]['name'] = $value['name_bn'] .'-' .$value['name'];
        //     $row1[$key]['text'] =$value['name_bn'] .'-' .$value['name'];
        ...
        foreach ($row as $key => $value) {
            ...
            $id = $value['id'];
            $status = $value['status'];
            $button = ($status == 1) ? "Deactive" : "Active";
            $row1[$key]['id'] = $value['id'];

            $edit_button = '  <a href="' . $base_url . 'Ccategory/category_update_form/' . $id . '" class="btn btn-primary  rounded-pill"' . "Edit" . '"><i class="fa fa-pencil" aria-hidden="true"></i><b style="margin-left:5px">Edit</b></a>';
            $delete_button = '  <a href="' . $base_url . 'Ccategory/category_delete/' . $id . '" class="btn btn-danger rounded-pill"' . "Delete" . '"><i class="fa fa-trash-o" aria-hidden="true"></i><b style="margin-left:5px">Delete</b></a>';
            $status_button = '  <a href="' . $base_url . 'Ccategory/update_category_status/' . $id . '/' . $status . '" class="btn btn-info rounded-pill"><i class="fa fa-trash-o" aria-hidden="true"></i><b style="margin-left:5px">' . $button . '</b></a>';

            $row1[$key]['name'] = $value['name_bn'] . '-' . $value['name'] . $edit_button . " " . $delete_button . " " . $status_button;
            $row1[$key]['text'] = $value['name_bn'] . '-' . $value['name'] . $edit_button . " " . $delete_button . " " . $status_button;
            $row1[$key]['nodes'] = array_values($this->membersTree($value['id']));
        }
    ...
    public function insert_category()
    {
        ...
        $data = array(
            ...
            'name_bn' => $this->input->post('name_bn', TRUE),
    ...
    public function update_category_status($id, $status)
    {
        ...
        $this->Categories->update_category($data, $id);
        $this->session->set_userdata(array('message' => display('successfully_updated')));
        redirect(base_url('Ccategory'));
    }
